...
namespace sparrow
{
    ...
    template <random_access_range B>
    constexpr dynamic_bitset_base<B>::iterator dynamic_bitset_base<B>::insert(const_iterator pos, value_type value)
    {
        return insert(pos, 1, value);
    }

    template <random_access_range B>
    dynamic_bitset_base<B>::iterator
    constexpr dynamic_bitset_base<B>::insert(const_iterator pos, size_type count, value_type value)
    {
    ...
    template <random_access_range B>
    constexpr dynamic_bitset_base<B>::iterator dynamic_bitset_base<B>::emplace(const_iterator pos, value_type value)
    {
    ...
    template <random_access_range B>
    constexpr dynamic_bitset_base<B>::iterator dynamic_bitset_base<B>::erase(const_iterator first, const_iterator last)
    {
        SPARROW_ASSERT_TRUE(cbegin() <= first);
        SPARROW_ASSERT_TRUE(first <= last);
        SPARROW_ASSERT_TRUE(last < cend());

        const size_type first_index = static_cast<size_type>(std::distance(cbegin(), first));
        const size_type last_index = static_cast<size_type>(std::distance(cbegin(), last));
    ...
    template <class B, bool is_const>
    constexpr auto bitset_iterator<B, is_const>::distance_to(const self_type& rhs) const noexcept -> difference_type
    {
