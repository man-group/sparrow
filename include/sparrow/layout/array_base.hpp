...
namespace sparrow
{
    ...
    template <class D>
    std::optional<const std::string_view> array_crtp_base<D>::name() const
    {
        return m_proxy.name();
    }

    template <class D>
    std::optional<const std::string_view> array_crtp_base<D>::metadata() const
    {
    ...
    constexpr unsigned long stoul(const std::string_view str)
    {
        ...
        while (i < str.size() && std::isdigit(static_cast<unsigned char>(str[i])))
        {
            ...
            if (result > (std::numeric_limits<unsigned long>::max() - digit) / 10)
            {
