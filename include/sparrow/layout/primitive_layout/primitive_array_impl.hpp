...
namespace sparrow
{
    ...
    )
    {
        ...
        return create_proxy(std::move(data_buffer), nullable, std::move(name), std::move(metadata));
    }
    ...
    {
        const std::size_t size = range_size(range);
        if (nullable)
        {
        ...
        else
        {
            ...
            std::vector<buffer<uint8_t>> buffers(2);
            buffers[0] = buffer<uint8_t>{nullptr, 0};  // no validity bitmap
            buffers[1] = details::primitive_data_access<T>::make_data_buffer(std::forward<R>(range))
                             .extract_storage();
    ...
    )
    {
        ...
        const auto flags = bitmap.has_value()
