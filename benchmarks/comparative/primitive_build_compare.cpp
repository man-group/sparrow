...
// Helper function to generate sequential test data
template <typename T>
...
// Helper function to generate nullable data with some null values
template <typename T>
std::vector<sparrow::nullable<T>> generate_nullable_data(
...
// Sparrow Array Creation Benchmarks
template <typename T>
...
// Sparrow Array Reading Benchmarks - Element Access
template <typename T>
...
static void BM_Arrow_ReadArrayElementAccess(benchmark::State& state) {
    ...
    for (auto _ : state) {
        if constexpr (std::is_same_v<T, std::int32_t>) {
            ...
            if (!typed_array->IsNull(index % size)) {
                // For bool, we don't accumulate but still access the value
                volatile bool val = typed_array->Value(index % size);
