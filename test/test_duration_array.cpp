...
namespace sparrow
{
    using testing_types = mpl::rename<duration_types_t, std::tuple>;
    ...
    std::vector<nullable<T>> make_nullable_values(size_t count)
    {
        ...
        {
            values.push_back(nullable<T>(T(i), i % 2));
        }
        return values;
    }
    ...
    TEST_SUITE("duration_array")
    {
        TEST_CASE_TEMPLATE_DEFINE("", T, duration_array_id)
        {
            ...
            SUBCASE("constructors")
            {
                ...
                SUBCASE("copy")
                {
                    const duration_array<T> ar(input_values);
                    const duration_array<T> ar2(ar);
                    CHECK_EQ(ar, ar2);
                }
                ...
                {
                    duration_array<T> ar(input_values);
                    const duration_array<T> ar2(std::move(ar));
            ...
            SUBCASE("operator[]")
            {
                ...
                SUBCASE("mutable")
                {
                    ...
                    std::vector<nullable<T>> new_values = [&input_values]()
                    {
                        ...
                        {
                            values.push_back(nullable<T>(T(i + 5), i % 2));
                        }
                        return values;
                    }();
            ...
            {
                duration_array<T> ar(input_values);
                const auto new_value = make_nullable<T>(T(99));
