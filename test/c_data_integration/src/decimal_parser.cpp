    ...
    )
    {
        if (nullable)
        {
            ...
            return sparrow::array{
                T{std::forward<DATA_RANGE>(data), std::move(validity), precision, scale, name, std::move(metadata)}
            };
        }
        else
        {
            return sparrow::array{
                T{std::forward<DATA_RANGE>(data), precision, scale, false, name, std::move(metadata)}
            };
        }
    }
