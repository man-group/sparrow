...
TEST_SUITE("arrow_length")
{
    TEST_CASE("is_valid_arrow_length")
    {
        ...
        if constexpr( sizeof(std::ptrdiff_t) <= max_arrow_length and config::enable_32bit_size_limit == false )
        {
            ...
            CHECK_FALSE(is_valid_arrow_length(std::numeric_limits<std::ptrdiff_t>::min()));
        }
        ...
        CHECK_FALSE(is_valid_arrow_length(std::ptrdiff_t(std::numeric_limits<std::int32_t>::min())));
    }
