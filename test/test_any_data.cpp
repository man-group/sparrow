...
TEST_SUITE("any_data")
{
    TEST_CASE("constructors")
    {
        ...
        {
            auto ptr = sparrow::arrow_array_shared_ptr{sparrow::default_arrow_array()};
            ptr->length = 99;
            ptr->null_count = 42;
            sparrow::any_data<ArrowArray> any_data(std::move(ptr));
        }
    ...
    TEST_CASE("get")
    {
        SUBCASE("mutable")
        {
            ...
            SUBCASE("arrow_array_shared_ptr")
            {
                ...
                ptr->null_count = 42;
                sparrow::any_data<ArrowArray> any_data(std::move(ptr));
                CHECK_EQ(any_data.get()->length, 99);
                CHECK_EQ(any_data.get()->null_count, 42);
            }
        ...
        SUBCASE("const")
        {
            ...
            SUBCASE("arrow_array_shared_ptr")
            {
                ...
                ptr->length = 99;
                ptr->null_count = 42;
                const sparrow::any_data<ArrowArray> any_data(std::move(ptr));
                CHECK_EQ(any_data.get()->length, 99);
                CHECK_EQ(any_data.get()->null_count, 42);
            }
    ...
    TEST_CASE("get_data")
    {
        SUBCASE("mutable")
        {
            ...
            SUBCASE("arrow_array_unique_ptr")
            {
                ...
                ptr->null_count = 42;
                sparrow::any_data<ArrowArray> any_data(std::move(ptr));
                [[maybe_unused]] const auto lol = any_data.type_id().name();
                using Type = sparrow::value_ptr<ArrowArray, sparrow::arrow_array_custom_deleter_struct>;
                REQUIRE_NOTHROW(any_data.get_data<Type&>());
                auto& data = any_data.get_data<Type&>();
                CHECK_EQ(data->length, 99);
                CHECK_EQ(data->null_count, 42);
            }
            ...
            SUBCASE("arrow_array_shared_ptr")
            {
                ...
                ptr->null_count = 42;
                sparrow::any_data<ArrowArray> any_data(std::move(ptr));
                REQUIRE_NOTHROW(any_data.get_data<sparrow::arrow_array_shared_ptr&>());
                auto& data = any_data.get_data<sparrow::arrow_array_shared_ptr&>();
                CHECK_EQ(data->length, 99);
                CHECK_EQ(data->null_count, 42);
            }
    ...
    TEST_CASE("type_id")
    {
        ...
        SUBCASE("ArrowArray")
        {
            ...
            CHECK_EQ(
                data.type_id(),
                typeid(sparrow::value_ptr<ArrowArray, sparrow::arrow_array_custom_deleter_struct>)
            );
        }
    }
