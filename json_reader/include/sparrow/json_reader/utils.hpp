...
{
    SPARROW_JSON_READER_API std::vector<std::byte> hex_string_to_bytes(const std::string& hex_string);
    SPARROW_JSON_READER_API std::vector<std::vector<std::byte>>
    hex_strings_to_bytes(const std::vector<std::string>& hexStrings);

    SPARROW_JSON_READER_API std::vector<nlohmann::json>
    get_children_with_same_name(const nlohmann::json& schema_or_array, const std::string& name);

    SPARROW_JSON_READER_API std::vector<bool> get_validity(const nlohmann::json& array);

    SPARROW_JSON_READER_API void check_type(const nlohmann::json& schema, const std::string& type);

    SPARROW_JSON_READER_API std::optional<std::vector<sparrow::metadata_pair>>
    get_metadata(const nlohmann::json& schema);

    SPARROW_JSON_READER_API std::vector<size_t> get_offsets(const nlohmann::json& array);

    SPARROW_JSON_READER_API std::vector<size_t> get_sizes(const nlohmann::json& array);
